...
const App_playlist = (props) => {
  ...
  const search = () => {
    if (searchRef.current.value && results && results.length != 0) {
    ...
    <>
      ...
      <section className='bg-white rounded-2xl m-auto w-3/4 mt-8 p-10 relative'>
        <section className='relative flex justify-center items-center mb-6'>
          ...
          />
          <button
            className='ml-5 bg-blue-600 py-2 px-3 text-lg rounded-lg text-white'
            onClick={() => {
        ...
        <h2>플레이리스트 노래 수 {results && results.length}</h2>
