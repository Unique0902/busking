  ...
  const search = () => {
    if (searchRef.current.value) {
      if (selectRef.current.value === '제목') {
        lastfm.searchSongByName(searchRef.current.value).then((result) => {
        ...
        lastfm.searchSongByArtist(searchRef.current.value).then((result) => {
          setSearchResults(result.trackmatches.track);
          setResultNum(parseInt(result['opensearch:totalResults']));
        });
      }
  ...
  return (
    <>
      ...
      <section className={styles.results}>
        ...
        <Page_num_screen resultNum={resultNum} pageNum={pageNum} />
      </section>
    </>
  );
};
