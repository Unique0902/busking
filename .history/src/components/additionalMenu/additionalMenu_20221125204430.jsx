...
const AdditionalMenu = ({ setIsShowAdditionalMenu }) => {
  ...
  return (
    ...
    >
      <section className=' border-b border-gray-600 border-solid flex flex-col pt-2'>
        ...
        <button className={`${btnStyle} mb-2`}>플레이리스트 제거</button>
      </section>
      <section className='flex flex-col py-2'>
        <p className='text-gray-500 font-sans text-base px-4 py-2'>
          모든 플레이리스트
        </p>
        <button className={btnStyle}>Playlist1</button>
        <button className={btnStyle}>Playlist2</button>
        <button className={`${btnStyle}`}>Playlist3</button>
      </section>
    </div>
  );
};
