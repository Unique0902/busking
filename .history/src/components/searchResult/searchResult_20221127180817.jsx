...
const SearchResult = ({ index, result, onSongClick, btnText }) => {
  ...
  return (
    <li className='flex flex-row w-full text-white font-sans font-light bg-black rounded-xl mb-4 text-lg justify-between px-2 py-2 text-center'>
      <p className='basis-1/12'>{index}</p>
      <p ref={nameRef} className='basis-7/12'>
        {result.name || result.title}
      </p>
      <p ref={artistRef} className='basis-1/4'>
        {result.artist.length > 8
          ? result.artist.substr(0, 8) + '..'
          : result.artist}
      </p>
