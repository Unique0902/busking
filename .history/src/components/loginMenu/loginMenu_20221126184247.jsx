...
const LoginMenu = ({ userData, logout, setIsShowLoginMenu }) => {
  ...
  const btnStyle = 'font-sans text-lg text-center py-1 px-4 hover:bg-gray-200';
  useEffect(() => {
  ...
  return (
    ...
    >
      <section className=' border-b border-gray-600 border-solid flex flex-col pt-2 pb-2'>
        ...
        <button className={`${btnStyle} text-black`}>회원정보</button>
        <button className={`${btnStyle} text-red-600`}>로그아웃</button>
      </section>
      <section className='flex flex-row justify-around py-2 px-4'>
