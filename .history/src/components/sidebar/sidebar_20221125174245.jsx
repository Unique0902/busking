...
const Sidebar = (props) => {
  ...
  const hideBtnStyle = 'text-white py-4 font-sans w-full font-medium text-lg';
      ...
      >
        {!isHide && (
          <div className='flex items-center py-3 px-5  border-solid text-white border-gray-600 border-b'>
            <FontAwesomeIcon
              icon={faBookOpen}
              className={`${iconStyle} text-2xl text-blue-500`}
            />
            <p className=' font-sans text-2xl font-semibold '>노래책</p>
          </div>
        )}
        {isHide && (
          <div className='flex items-center text-blue-500 justify-center text-2xl w-full text-center py-4 border-solid border-gray-600 border-b'>
        ...
        <ul>
          ...
          <li>
            {isHide ? (
              <button
                className='absolute border-solid border-gray-600 border-t bottom-0 py-7 w-full text-right text-white'
                onClick={() => {
                  setIsHide(false);
                }}
              >
